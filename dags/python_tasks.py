...
def feature_extraction():
    ...
    memes['ref_site'] = memes['additional_references'].apply(lambda x: list(eval(x).keys()) if x else np.nan)
    memes['ref_url'] = memes['additional_references'].apply(lambda x: list(eval(x).values()) if x else np.nan)
...
def list_clean():
    ...
    memes['search_keywords'] = memes['search_keywords'].apply(lambda row: ','.join([i.strip() for i in row.split(',') if i]) if type(row) == str else row)


    memes['tags'] = memes['tags'].apply(lambda row: clean.sub(' ', row))
    memes['tags'] = memes['tags'].apply(lambda row: ','.join([i.strip() for i in row.split(',') if i]))
    memes['tags'] = memes['tags'].apply(lambda x: x if x != '' else np.nan)

    memes['siblings'] = memes['siblings'].apply(lambda row: re.sub(r"[\[\]']", "", row) if type(row) == str else row)
    memes['children'] = memes['children'].apply(lambda row: re.sub(r"[\[\]']", "", row) if type(row) == str else row)
    memes['ref_site'] = memes['ref_site'].apply(lambda row: re.sub(r"[\[\]']", "", row) if type(row) == str else row)
    memes['ref_site'] = memes['ref_site'].apply(lambda row: row.replace(',,', ',') if type(row) == str else row)
